  ...
  while True:
    ...
    if len(v) == len(case):
      if v != case and v != tuple(reversed(case)):
        msg = WRONG_ORDER_ERROR + " You guessed:\n"
        msg += str(v) + '\n'
        msg += "One of these two:\n"
        msg += str(case) + '\n'
        msg += str(list(reversed(case)))
        raise Error(msg)
      return q
    if q >= max_q:
      raise Error(TOO_MANY_QUERIES_ERROR + ' ' + str(q) + ' ' + str(max_q))
    q += 1
    Output(Mid(pos, v))
